...
// проверяет создание пользоваетелй, Получение информации о пользователях по ID, Проверку авторизации через эндпоинт "hello"
public class UserControllerIntegrationTest extends BaseIntegrationTest {
    //Мокируем UserService, чтобы изолировать тестируемый код и проверить, как контроллер взаимодействует с сервисом
    @MockBean
    ...
    class CreateUser {
        ...
        //Проверяет, что контроллер возвращает ошибку 400 (Bad Request) при некорректных данных, проходит через раз из-за контейнеров в UserRequest
        @Test
        @WithMockUser(roles = {"MODERATOR"})
    ...
    class GetUserByIdTest{
        ...
        // проверяет обработку некорректного ID
        @Test
        @WithMockUser(roles = "MODERATOR")
    ...
    // проверка работы энд поинта hello
    @Nested
    class HelloTest{
        ...
        //проверяет, если пользователь без роли MODERATOR получает ошибку Forbiden 403
        @Test
